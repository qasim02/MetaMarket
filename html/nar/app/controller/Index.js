/* Controller per le interazioni con l'Indice */
Ext.define('NR.controller.Index', {
	extend : 'Ext.app.Controller',
	
	id : 'indexController',
	
	/* Funzione di inizializzazione per gli handler di eventi (click sulla categoria) */
	init : function() {
		this.control({
			'index' : {
	...
	/* Metodo per aggiungere la categoria selezionata al descrittore cosa-faccio-oggi */
	...
	/* Metodo per rimuovere una categoria se ci si clicca due volte */
	...
	/* metodo che "riempie" la tabella centrale con le location della categoria selezionata e aggiunge la categoria selezionata a cosa-faccio-oggi */
		/* Innanzitutto aggiungo la categoria selezionata alle categorie di cosa-faccio-oggi */
		this.addCategory(grid,record);
		/* Poi uniformo la stringa della categoria richiesta */
		var selected = record.get('category');
		selected.toLowerCase();
		selected.replace('/', '%2F');
		/* Salvo la View tabellare e ne modifico lo store in modo da immagazzinare le location */
		var locationView = Ext.getCmp('locationView');
		var locationStore = locationView.getStore();
		/* Pulisco lo store altrimenti sovrappongo le operazioni eseguite dal listener dichiarato in fondo (onLoad) */
		locationStore.removeAll(false);
		/* Configuro il proxy per la richiesta asincrona */
		locationStore.setProxy({
			/* Le location sono richieste dinamicamente al descrittore "tipologia" */
			type : 'ajax',
			...
			failure : function(){
				Ext.MessageBox.alert('Errore','Qualcosa è andato storto! Si prega di riprovare più tardi.');
			}
		});
		/* Devo mostrare i marker relativi alla categoria selezionata SOLO se sono nella prima tab (che ha xtype location)! */
		locationStore.on('load', function() {
			/* Appena lo store si carica mostra tutte le location sulla mappa */
			var selectedTab = Ext.getCmp('centralContent').getActiveTab().xtype; // è la proprietà che definisce l'ordine delle tab
			if (selectedTab == 'location'){
				var descrBox = Ext.getCmp('descrBox');
				/* Abilito e mostro il pannello di descrizione se precedentemente nascosto o disabilitato */
				descrBox.expand();
