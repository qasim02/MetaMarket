...
Ext.define('NR.view.Geolocal', {
	...
	/* Metodo per spostare la mappa sul punto indicato da lat e lng */
	panAndMark : function(lat, lng){
		/* marker : {title : 'La tua posizione (approssimativa)', icon : this.icons["Position"] } */
		if (!this.getMap()) this.setTheCenter(lat, lng);
		/* Se è già stata inizializzata la mappa la sposto sul punto indicato e piazzo un marker */
		var thisgmap = this.getMap();
		var coordObj = new google.maps.LatLng(lat, lng);
		thisgmap.panTo(coordObj);
	},
