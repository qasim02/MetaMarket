...
Ext.define('NR.view.Geolocal', {
	...
	markers : new Array(),		// Location aggiunte finora
	infowindows : new Array(),	// Infowindow aggiunte finora
	...
	setCenter : function(lat, lng){
		...
		this.setCenter = {
			lat : lat,
			lng : lng,
			marker : {title : 'La tua posizione (approssimativa)', icon : this.icons["Position"] }
		};
	},
	...
	/* 
	 * Agiungo il marker relativo alla location selezionata con infowindow 
	 */
	addAMarker : function(grid, record, multi, ordered){		
		...
		var dragValue = true;
		// Catturo la mappa sia come google.maps.Map che come Ext.ux.GMapPanel per essere pi� flessibili
		...
		/* Formo l'icona del marker in base alla categoria e/o alla numerazione, 
		 * se tra quelle suggerite, usando la categoria del record o this.locNum
		 */
		var iconURL = '';
		if (!ordered) {
	...
	setStreetView : function(record){
		...
		var locId = record.get('id');
		/* Segno che ho gia inizializzato la descrizione per il marker */
		...
		Ext.Ajax.request({
			url: 'http://ltw1216.web.cs.unibo.it',
	...
	/* Metodo per ottenere la distanza della location selezionata dalla posizione specificata */		
		Ext.Ajax.request({
			url: 'http://ltw1216.web.cs.unibo.it/distanza/params/'+lat1+'/'+long1+'/'+lat2+'/'+long2+'/',
			success: function (response) {
	...
	/* Metodo per ottenere la distanza della location selezionata dalla posizione specificata */
		...
		/* Aggiungo uno 0 iniziale se il valore e compreso tra 0 e 9 */
		if (hours < 10) {
			hours = '0'+hours;
		}
		if (minutes < 10) {
		...
		Ext.Ajax.request({
			...
			success: function (response) {
                ...
                switch (opening) {				
					case '0' :
						opening = '<font color="red"> E chiuso </font>';
						break;
					case '1' :
						opening = '<font color="yellow"> Sta per aprire </font>';
						break;
					case '2' :
						opening = '<font color="green"> E aperto </font>';
						break;
					case '3' :
						opening = '<font color="orange"> Sta per chiudere </font>';
						break;
					case '4' :
						opening = '<font color="yellow"> Sta per chiudere ma riaprira piu tardi </font>';
						break;			
					default : 
						opening = '<font color="red">Non disponibile</font>';
						break;
				}
				infowindow.appendContent('<p><b>E aperto nei prossimi '+delay+' minuti?</b><i> '+opening+'</i></p><br/>');
				infowindow.close();
